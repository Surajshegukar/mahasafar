...
export default function CardSlider({ attractions,title }) {
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex space-x-2">
          <button 
            ...
            className={`p-2 rounded-full ${scrollPosition <= 0 ? 'bg-gray-200 text-gray-400' : 'bg-white text-gray-800 shadow hover:bg-gray-100'}`}
            aria-label="Previous"
          >
          ...
          <button 
            ...
            className={`p-2 rounded-full ${scrollPosition >= maxScroll ? 'bg-gray-200 text-gray-400' : 'bg-white text-gray-800 shadow hover:bg-gray-100'}`}
            aria-label="Next"
          >
      ...
      >
            ...
            >
              <div className="border border-gray-200 rounded-lg p-4 bg-white h-full transition-transform duration-300 hover:shadow-md hover:scale-102 transform">
                ...
                <div className="flex items-center mt-1">
                  ...
                  <span className="text-xs font-medium ml-1">{attraction.rating}</span>
                  <span className="text-yellow-500 ml-1">⭐</span>
                </div>
                <p className="text-xs text-gray-600 mt-1">
                  Price: {attraction.price || "N/A"}
                </p>
                <a
        ...
        <div className="flex justify-center mt-4 space-x-1">
          {attractions.length > 0 && 
            Array(Math.min(Math.ceil(attractions.length / 3), 6)).fill().map((_, i) => (
              <div 
                key={i}
                className={`h-1 rounded-full transition-all duration-300 ${
                  i * 3 * cardWidth <= scrollPosition && scrollPosition < (i + 1) * 3 * cardWidth
                    ? 'w-4 bg-blue-500' 
                    : 'w-2 bg-gray-300'
                }`}
              />
...
export function TopAttractionsSection({ TraveAdvisor }) {
  if (!TraveAdvisor || !TraveAdvisor.attractions || TraveAdvisor.attractions.length === 0) {
    return null;
  }
