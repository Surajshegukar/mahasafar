...
import { MapPin, Clock, Coffee, Utensils, Compass, Bed, Activity } from 'lucide-react';
...
export default function ItineraryTimeline({ tripData, expandedSection }) {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const dayPositions = tripData.dailyItinerary.map(day => {
      ...
      // Find the day whose top is closest but not below the middle of the screen
                      ...
                      <div className="mb-6">
                        ...
                        <ul className="space-y-1 text-sm text-gray-700 pl-8">
                          {day.foodSuggestions.map((food, index) => (
                            <li key={index} className="mb-1">{food}</li>
                          ))}
                        </ul>
                      </div>
                    )}
